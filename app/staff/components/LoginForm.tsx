...
import { errorType, useAuthStaff } from '@/services/hooks/auth/hook';
...
const LoginForm = () => {
  ...
  useEffect(() => {
    if (isError) {
      ...
      document.cookie = `staffToken=${data.accessToken}; max-age=86400; path=/; secure;`;
      document.cookie = `tokenOwner=${currentCampaign}; max-age=86400; path=/; secure;`;

      router.push(`/staff/${currentCampaign}`);
    }
  ...
  return (
    <form className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md mx-auto space-y-6" onSubmit={handleSubmit}>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm mt-2">Access your staff dashboard</p>
      </div>
