...
const CampaignContent = () => {
  ...
  return (
    <section className="flex flex-col sm:flex-row justify-center sm:items-center bg-white gap-4 mt-10">
      {contents.map((content, i) => {
        const { image, title, description } = content;
        return (
          <div
            key={i}
            className="w-full sm:w-[20rem] p-2 cursor-pointer flex flex-col gap-6"
          >
            <Image
              src={image}
              alt="placeholder"
