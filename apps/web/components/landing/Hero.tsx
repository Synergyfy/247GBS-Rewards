...
import { ArrowRight, Sparkles, Zap, TrendingUp, CheckCircle, Sun, Leaf, Snowflake, Flower2, Gift, Percent, Users, Star, Play, X, Calendar } from 'lucide-react';
import SnowfallBackground from './seasonal/SnowfallBackground';
import FallingLeavesBackground from './seasonal/FallingLeavesBackground';
import SakuraBackground from './seasonal/SakuraBackground';
...
const seasons = [
  {
    ...
    phoneBg: 'bg-gradient-to-br from-amber-400/80 to-orange-500/80',
    phoneStats: { start: '21 June', end: '22 Sept', growth: '+34%' },
    phoneNotif: 'Ice Cream Promo Active!',
    btnTextColor: 'text-white',
    bgImage: '/images/seasons/summer.png'
  },
  {
    id: 'autumn',
    name: 'Autumn',
    ...
    phoneBg: 'bg-gradient-to-br from-orange-500/80 to-red-600/80',
    phoneStats: { start: '23 Sept', end: '20 Dec', growth: '+42%' },
    phoneNotif: 'Autumn VIP Unlocked!',
    btnTextColor: 'text-white',
    bgImage: '/images/seasons/autumn.png'
  },
  {
    id: 'winter',
    name: 'Winter',
    ...
    phoneBg: 'bg-gradient-to-br from-blue-500/80 to-indigo-600/80',
    phoneStats: { start: '21 Dec', end: '19 Mar', growth: '+67%' },
    phoneNotif: 'Holiday Gift Card Sent!',
    btnTextColor: 'text-white',
    bgImage: '/images/seasons/winter.png'
  },
  {
    id: 'spring',
    name: 'Spring',
    ...
    colorAccent: 'bg-emerald-950',
    colorText: 'text-green-600',
    colorBgLight: 'bg-green-50',
    colorBorder: 'border-green-100',
    phoneBg: 'bg-gradient-to-br from-green-400/80 to-emerald-500/80',
    phoneStats: { start: '20 Mar', end: '20 June', growth: '+28%' },
    phoneNotif: 'Spring Promo Live!',
    btnTextColor: 'text-white',
    bgImage: '/images/seasons/spring.png'
  },
];
...
const getCurrentSeasonId = () => {
  ...
  if ((month === 11 && day >= 21) || month === 0 || month === 1 || (month === 2 && day <= 19)) {
    return 'winter';
  }
...
const Hero = () => {
  ...
  // Dynamic seasons ordering
  const currentSeasonId = React.useMemo(() => getCurrentSeasonId(), []);

  const orderedSeasons = React.useMemo(() => {
    const current = seasons.find(s => s.id === currentSeasonId);
    const others = seasons.filter(s => s.id !== currentSeasonId);
    return current ? [current, ...others] : seasons;
  }, [currentSeasonId]);
  ...
  // Auto-rotate seasons with bias for the current season
  useEffect(() => {
    const currentSeason = orderedSeasons[currentIndex];
    const duration = currentSeason.id === currentSeasonId ? 12000 : 6000;

    const timeout = setTimeout(() => {
      setCurrentIndex((prev) => (prev + 1) % orderedSeasons.length);
    }, duration);

    return () => clearTimeout(timeout);
  }, [currentIndex, orderedSeasons, currentSeasonId]);
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen pt-28 pb-20 lg:pt-36 lg:pb-32 overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-100 z-[1]" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <motion.div style={{ y, opacity }} className="text-left">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex items-center gap-2 mb-6">
                  <div className={`p-2 rounded-xl ${currentSeason.colorBgLight} ${currentSeason.colorText}`}>
                    <currentSeason.icon className="w-5 h-5" />
                  </div>
                  <span className={`text-sm font-black uppercase tracking-widest ${currentSeason.colorText}`}>
                    {currentSeason.name} Edition
                  </span>
                </div>
            ...
            {/* Season Navigation Pills - Moved from top to replace mini features */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          >
            ...
            {/* CSS Phone Mockup */}
            <div className="relative z-20 w-[280px] sm:w-[320px] h-[560px] sm:h-[640px] bg-slate-900 rounded-[3rem] p-3 sm:p-4 shadow-2xl shadow-slate-900/30 border-[6px] border-slate-800 ring-1 ring-slate-700">
              {/* Screen Content */}
              <div className="w-full h-full bg-slate-50 rounded-[2.2rem] overflow-hidden relative flex flex-col z-10">
                {/* Status Bar */}
                <div className="absolute top-0 w-full h-6 z-40 flex justify-between px-6 items-center text-[10px] font-bold text-slate-800">
                  <span>9:41</span>
                ...
                {/* Notch */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/3 h-6 bg-slate-900 rounded-b-xl z-40" />
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      style={{
                        backgroundImage: `url(${(currentSeason as any).bgImage})`,
                    ...
                    {/* Seasonal Effects Overlay inside Phone */}
                    <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
                      {currentSeason.id === 'winter' && <SnowfallBackground color="bg-blue-300" />}
                      {currentSeason.id === 'autumn' && <FallingLeavesBackground />}
                      {currentSeason.id === 'spring' && <SakuraBackground />}
                    </div>

                    <div className="relative z-20 h-full flex flex-col">
                      {/* Header */}
                      <div className={`${currentSeason.phoneBg} backdrop-blur-md h-40 rounded-b-[2rem] p-4 pt-8 relative overflow-hidden border-b border-white/20`}>
                        <div className={`absolute top-0 right-0 w-28 h-28 bg-white/20 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl`} />
                        <div className="flex justify-between items-center mb-2">
                          <div className="w-8 h-8 rounded-full bg-white/30 backdrop-blur-md flex items-center justify-center">
                            <currentSeason.icon className="w-4 h-4 text-white" />
                          </div>
                          <div className="w-6 h-6 rounded-full bg-white/30 backdrop-blur-md" />
                        </div>
                        <h3 className="text-xl font-extrabold leading-tight text-white drop-shadow-md">Happy {currentSeason.name}!</h3>
                        <p className="text-white/90 text-[11px] font-bold mt-0.5">{currentSeason.dateRange}</p>
                      </div>

                      {/* Stats Grid */}
                      <div className="px-5 -mt-6 relative z-10 grid grid-cols-2 gap-3">
                        <div className="bg-white/80 backdrop-blur-xl p-3 rounded-xl shadow-xl shadow-black/10 flex flex-col gap-1.5 border border-white/40">
                          <div className={`w-7 h-7 rounded-lg ${currentSeason.colorBgLight} flex items-center justify-center ${currentSeason.colorText}`}>
                            <Calendar size={14} />
                          </div>
                          <span className="text-[10px] text-slate-600 font-bold uppercase tracking-tighter">Season Starts</span>
                          <span className="text-sm font-black text-slate-900 leading-none">{currentSeason.phoneStats.start}</span>
                        </div>
                        <div className="bg-white/80 backdrop-blur-xl p-3 rounded-xl shadow-xl shadow-black/10 flex flex-col gap-1.5 border border-white/40">
                          <div className={`w-7 h-7 rounded-lg ${currentSeason.colorBgLight} flex items-center justify-center ${currentSeason.colorText}`}>
                            <Zap size={14} />
                          </div>
                          <span className="text-[10px] text-slate-600 font-bold uppercase tracking-tighter">Season Ends</span>
                          <span className="text-sm font-black text-slate-900 leading-none">{currentSeason.phoneStats.end}</span>
                        </div>
                      </div>

                      {/* Chart Area */}
                      <div className="px-5 mt-4">
                        <div className="bg-white/80 backdrop-blur-xl rounded-xl p-4 border border-white/40 shadow-xl shadow-black/5">
                          <div className="flex justify-between items-end h-20 gap-1.5">
                            {[35, 55, 45, 70, 50, 85, 65].map((h, i) => (
                              <motion.div
                                ...
                                className={`w-full ${currentSeason.colorAccent} rounded-t-sm opacity-90`}
                              />
                            ))}
                          </div>
                      ...
                      <motion.div
                        ...
                        className={`absolute bottom-16 right-4 bg-white/90 backdrop-blur-xl p-3 rounded-xl shadow-2xl border-white border z-20 max-w-[130px]`}
                      >
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <div className={`text-[10px] font-bold ${currentSeason.colorText} leading-tight`}>{currentSeason.phoneNotif}</div>
                        </div>
                      ...
                      <motion.div
                        ...
                        className={`absolute bottom-16 left-4 ${currentSeason.colorAccent} text-white text-[10px] font-black px-2.5 py-1 rounded-full shadow-lg flex items-center gap-1 z-30`}
                      >
                ...
                {/* Bottom Bar */}
                <div className="h-1 bg-slate-900 mx-auto w-1/3 rounded-full absolute bottom-1.5 left-1/2 -translate-x-1/2 z-40" />
              </div>

              {/* Physical Buttons */}
              <div className="absolute top-20 -left-[2px] w-[2px] h-7 bg-slate-700 rounded-l-md z-30" />
              <div className="absolute top-32 -left-[2px] w-[2px] h-12 bg-slate-700 rounded-l-md z-30" />
              <div className="absolute top-32 -right-[2px] w-[2px] h-12 bg-slate-700 rounded-r-md z-30" />
            </div>
          </motion.div>
